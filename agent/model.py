...
class QNetwork(nn.Module):
    def __init__(self, action_size=7, input_channels=4):
        ...
        self.fc1 = nn.Linear(64 * 11 * 11 + 4, 512)
    ...
    def forward(self, img, infos):
        x = self.relu(self.conv1(img/255.0))
...
class DDQNAgent(DQNAgent):
    ...
    def update_model(self, batch_size):
        ...
        current_q_values = self.q_network(stacked_frames_batch, state_infos_batch).gather(1, action_batch.unsqueeze(0))
        next_actions = self.target_q_network(next_stacked_frames_batch, next_state_infos_batch).max(1).indices
        expected_q_values = reward_batch + self.gamma * self.q_network(next_stacked_frames_batch, next_state_infos_batch).gather(1, next_actions.unsqueeze(0))
